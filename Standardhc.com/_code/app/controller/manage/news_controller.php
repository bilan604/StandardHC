 ...
 *
 *
 * Controller_Manage_News 控制器
 */
class Controller_Manage_News extends Controller_Manage_Abstract
{
	...
	protected function _config()
	{
		return array(
			...
			"list" => array(
				...
				"default_ot" => "DESC",
				"list_fields" => "page_title,page_intro,page_status,page_publish,page_updated",
				"list_fields_name" => "标题,内容摘要,显示,发布时间,更新时间",
				"list_num" => 20,
				"list_filter" => array(
					"page_updated" => "date",
					'page_status' => 'boolean',
...
/* ******************************************************************** */
	...
	function actionIndex()
	{
		...
		$frm=Frame::find('frame_name=? and language=?','article',$lang)->query();
		$page_tree = $this->_context->page_tree;//dump($frm);exit;
		$this->_view['frm'] = $frm;
	...
	function actionEdit()
	{
		...
		if ($id)
		{
			...
			if (!$instance->id())
			{
				...
				return $this->_redirect(url("manage::news",array('page_tree'=>$page_tree)));
			}
			$edit_mode = true;
		...
		//表单初始化
		$form = new Form_Upload('Form_Page', url("manage::news/edit") );
		...
		if( $this->_context->isPOST() && $form->validate($_POST,$failed) )
		{
			try
			{
				...
				{
					$_redirect_to = url("manage::news/edit",array('page_tree'=>$page_tree) );
				}
				elseif ($this->_context->_continue != '')
				{
					$_redirect_to = url("manage::news/edit",array('page_tree'=>$page_tree,'id'=>$instance['pid']));
				}
				else
				{
					$_redirect_to = url("manage::news",array('page_tree'=>$page_tree) );
				}
	...
	function actionHot()
	{
		...
		try
		{
			...
			{
				$sta = 1-$item->page_hot;
				$meta->updateWhere(array('page_hot'=>$sta),$idname . "=? ", $item[$idname]);
			}
			Helper_Session::Set('notice',$config['rowname'] . " 隐藏/显示操作成功");
		}
		catch (Exception $ex)
	...
	protected function _afterFormInit(QForm $form)
	{
		...
		//$form['page_hot']->items = array('0'=>'否','1'=>'是');
