<?php
// $Id: Permissions.php @2009 ccbox.net 1:22 2009-5-14 $
/**
 * 列出控制器目录中的所有控制器及方法的类
**/
        ...
        foreach ($ctrl_files as $filename=>$value) {
            ...
            if(isset($matched[1][0])){
                ...
                if (stripos( $filename , '/')){
                    ...
                    $ctrl_files[$filename]['controller'] = strtolower($nsctl[1]);
                    ...
                    $ctrl_files[$filename]['controller'] = strtolower($matched[1][0]);
                }
            }
            ...
            if(isset($matched[1])){
                foreach ($matched[1] as $value){
                    $ctrl_files[$filename]['action'][] = strtolower($value);
                }
                foreach ($matched[2] as $value){
                    $pattern2 = '/\/\/@title\s=\s(.+?)/';
					//$pattern2 = '/show_box\[\'title\'\]\s=\s\'(.+?)\'/';
    ...
    function updatePermissions($updateCtrlDir = '') {
        if (is_dir($updateCtrlDir) && stripos( $updateCtrlDir , '/app/controller/')) {
            ...
            foreach ($dbActions as $oneActions) {
                $newDbActions[$oneActions['pid']] = $oneActions['namespace']."_".$oneActions['controller']."_".$oneActions['action'];
            }
            ...
            if (count($delPermissions)){
                ...
                foreach ($delPermissions as $key => $delPermission) {
                    Permissions::meta()->destroyWhere('pid = ?',$key);
                }
            }
            return true;
        }else{
