...
class MyApp
{
    ...
    protected function __construct(array $app_config)
    {
       ...
       // set_magic_quotes_runtime(0);
		...
		// 多语言支持
		require_once $app_config['ROOT_DIR'] . '/_code/lib/qtranslate.php';
		//多语言
    ...
    function currentUserObject()
    {
        ...
        $user = User::find('uid = ?', $data['uid'])->query();
        if ($user->id())
        {
            return $user;
        }
    ...
    static function loadConfigFiles(array $app_config)
    {
        ...
        (
			...
			"{$cfg}/modules.{$ext}"                    => 'modules', /* 模块配置文件 */
        );
	...
	protected function _on_access_denied()
    {
		...
		if (empty($roles))
		{
			...
			if ($namespace == 'manage')
				return new QView_Redirect(url('manage::default/login'));
			else
				return new QView_Redirect(url('default::user/signin'));
