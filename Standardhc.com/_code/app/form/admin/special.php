<?php
class Form_Admin_Special extends QForm
{
    /**
    ...
    {
		$config_name = 'special_form.yaml';
		
        $form = new Form_Admin_Special('Form_Admin_Special', $action);
        $filename = rtrim(dirname(__FILE__), '/\\') . DS . $config_name;
        $form->loadFromConfig(Helper_YAML::loadCached($filename));
		$form->element('categoryid')->items = Category::find()->getAll()->toHashMap('id','en_name');
        $form->addValidations(Special::meta());
