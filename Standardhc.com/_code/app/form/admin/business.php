<?php
class Form_Admin_Business extends Form_Upload
{
    /**
    ...
    {
		$config_name = 'business_form.yaml';

        $form = new Form_Admin_Business('Form_Admin_Business', $action);
        $filename = rtrim(dirname(__FILE__), '/\\') . DS . $config_name;
        $form->loadFromConfig(Helper_YAML::loadCached($filename));
        $form->addValidations(Business::meta());
