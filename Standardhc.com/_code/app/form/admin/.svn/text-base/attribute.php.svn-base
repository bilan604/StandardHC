<?php
class Form_Admin_Attribute extends QForm
{
    /**
    ...
    {
		$config_name = 'attribute_form.yaml';

        $form = new Form_Admin_Attribute('Form_Admin_Attribute', $action);
        $filename = rtrim(dirname(__FILE__), '/\\') . DS . $config_name;
        $form->loadFromConfig(Helper_YAML::loadCached($filename));
        $form->addValidations(Attribute::meta());
